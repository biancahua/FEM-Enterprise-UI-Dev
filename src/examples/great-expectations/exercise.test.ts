...
// it.todo and it.skip are generally treated the same; semantic differences
// .todo is usually used when your test is incomplete and change it to .skip when moved to production IF you don't want to use the test?
it(
...
describe('Kanban Board', () => {
  it('should include "Backlog" in board.statuses', () => {
    ...
    // Verify that board.statuses contains "Backlog". strings use toInclude and arrays use toContain?
  ...
  it(
    ...
    () => {
      ...
      board.addStatus('Hellow')
      // Verify that the new status is—in fact—now in board.statuses.
      expect(board.statuses).toContain('Hellow')
    },
  );
